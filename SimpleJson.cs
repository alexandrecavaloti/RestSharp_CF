 ...
 static class SimpleJson
    {
                    ...
                    else if (c == 'u')
                    {
                        ...
                        if (remainingLength >= 4)
                        {
                            ...
                            if (0xD800 <= codePoint && codePoint <= 0xDBFF)  // if high surrogate
                            {
                                ...
                                {
                                    uint lowCodePoint;
                                    lowCodePoint = UInt32.Parse(new string(json, index + 2, 4), NumberStyles.HexNumber, CultureInfo.InvariantCulture);
                                    bool sucessoLowcode = lowCodePoint > 0;
                                    if (new string(json, index, 2) == "\\u" && sucessoLowcode)
                                    {
        ...
        static object ParseNumber(char[] json, ref int index, ref bool success)
        {
            ...
            {
                long number;
                number = long.Parse(new string(json, index, charLength), NumberStyles.Any, CultureInfo.InvariantCulture);
